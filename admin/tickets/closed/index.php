...
if ($queryticket = mysqli_query ($connect, "SELECT * FROM `user_tickets` WHERE `nr` = '$nr'")) {
  while ($ticket = mysqli_fetch_assoc($queryticket)) {
   ...
   echo "<p class='text-center'><hr><p class='text-left'><b>".wordwrap(substr(strip_tags($ticket['content']), 0, 262), 39, "\n", true)."</b></p><p class='text-right'><i>".$ticket['date']."</i> Author: <b><a href='/users.php?nick=$nick'>".substr(strip_tags($ticket['nick']), 0, 22)."</b></a><p class='text-right'>Status: <b>".$ticket['status']."</b> | <b><a href='delete.php?nr=$nr'>[ DELETE ]</a></b></p>\n<hr><h2 class='text-center'>Comments</h2>";
...
<div class="container">
   <div>
   <h1 class="text-center">Closed tickets</h1>
      <div>
         <div>
            <div>
			<hr>
               <?php
               $login = $_SESSION['login'];
                  if ($query = mysqli_query($connect, "SELECT * FROM `user_tickets` WHERE `status` = 'closed' ORDER BY `nr` DESC")) {
