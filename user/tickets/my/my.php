               ...
               $login = $_SESSION['login'];
                  if ($query = mysqli_query($connect, "SELECT * FROM `user_tickets` WHERE `nick`='$login' ORDER BY `nr` DESC")) {
                     while ($my = mysqli_fetch_assoc ($query)) {
                       ...
                       $login = $_SESSION['login'];
                       $nick = $my['nick'];
                       echo "<a href='my.php?show=$nr'><p class='text-left'><b>".substr(strip_tags($my['title']), 0, 32)."</a></b></p><p cliass='text-right'><i>".strip_tags($my['date'])."</i> Author: <b><a href='users.php?nick=$nick'>".substr(strip_tags($my['nick']), 0, 22)."</a></b></i></b></p><p class='text-right'>Status: <b>".$my['status']."</b></p>\n<hr>";
                  }
               }
