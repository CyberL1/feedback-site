  ...
  while ($ticket = mysqli_fetch_assoc($queryticket)) {
   ...
   if ($query = mysqli_query($connect, "SELECT * FROM `user_tickets` WHERE `nr`='$nr'")) {
      while ($auth = mysqli_fetch_assoc ($query)) {
         $nr = $auth['nr'];
         if($ticket['nick'] !== $_SESSION['login']) {
            echo '<p>You cannot see other users ticket</p>';
            mysqli_close($connect);
            exit();
         } else {
   echo "<h1 class='text-center'>".wordwrap(substr(strip_tags($ticket['title']), 0, 99), 19, "\n", true)."</h1>";
   echo "<p class='text-center'><hr><p class='text-left'><b>".wordwrap(substr(strip_tags($ticket['content']), 0, 262), 39, "\n", true)."</b></p><p class='text-right'><i>".$ticket['date']."</i> Author: <b><a href='/users.php?nick=$nick'>".substr(strip_tags($ticket['nick']), 0, 22)."</b></a><p class='text-right'>Status: <b>".$ticket['status']."</b> | <b><a href='comment?nr=$nr'>[ COMMENT ]</a></b></p>\n<hr><h2 class='text-center'>Comments</h2>";
      ...
      <div>
         <div>
			...
			<hr>
               <?php
               $login = $_SESSION['login'];
                  if ($query = mysqli_query($connect, "SELECT * FROM `user_tickets` WHERE `nick`='$login' ORDER BY `nr` DESC")) {
                     while ($my = mysqli_fetch_assoc ($query)) {
                       $nr = $my['nr'];
                       $login = $_SESSION['login'];
                       $nick = $my['nick'];
                       echo "<a href='?show=$nr'><p class='text-left'><b>".substr(strip_tags($my['title']), 0, 32)."</a></b></p><p class='text-right'><i>".strip_tags($my['date'])."</i> Author: <b><a href='users.php?nick=$nick'>".substr(strip_tags($my['nick']), 0, 22)."</a></b></i></b></p><p class='text-right'>Status: <b>".$my['status']."</b></p>\n<hr>";
                  }
               }
